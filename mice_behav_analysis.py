...
def set_paths(path_ops):
    ...
    elif path_ops == 'molano':
        PATH =\
            '/home/molano/Dropbox/project_Barna/mice_data/standard_training_2020/'
        SV_FOLDER = PATH
...
def learned_categories(sbj, df, index_event=None, color_ev='', verbose=True,
    ...
    def get_trial_info(df, subj):
        ...
        Returns
        -------
        For each mouse, it returns a list of the accuracies, the ground-truth sides
        the sessions in each stage and the colors of each stage.
...
def plot_accuracy_trials_subj_stage4(hit, xs, col, ax, subj):
    """
    Plot accuracy over session for every subject, showing the stages the mice are
    in different colors.
